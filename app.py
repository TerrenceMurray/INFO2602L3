...
def signup_user_view():
  data = request.json
  try:
    new_user = RegularUser(data['username'], data['email'], data['password']) # type: ignore
...
def create_todo_view():
  ...
  new_todo = user.add_todo(data['text']) # type: ignore
...
def edit_todo_view(id):
  ...
  user.update_todo(id, data['text']) # type: ignore
  return jsonify(message=f"todo updated to '{data['text']}'!"), 200 # type: ignore
